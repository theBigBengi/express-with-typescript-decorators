...
function routeBinder(method) {
    return function (path) {
        return function (originalMethod, context) {
            // Path metadata
            Reflect.defineMetadata(MetadataKeys_1.MetadataKeys.path, path, originalMethod, context.name);
            //MethodPath metadata
            Reflect.defineMetadata(MetadataKeys_1.MetadataKeys.method, method, originalMethod, context.name);
        };
    };
}
